...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for key, value in results_stats_dic.items():
        if "pct_" in key:
            print("{} = {}".format(key.replace("pct_", "% ").replace("_", " ").title(), value))
    ...
    if print_incorrect_breed:
        if results_stats_dic["n_correct_breed"] == results_stats_dic["n_correct_dogs"]:
            print("\nMisclassified Breeds: None")
        else:
            print("\nMisclassified Breeds:")
            for filename, labels in results_dic.items():
                if labels[2] == 0 and sum(labels[3:]) == 2:
                    print(filename)